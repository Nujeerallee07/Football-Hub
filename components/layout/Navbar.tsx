...
export function Navbar() {
  ...
  const { mobileOpen, setMobileOpen } = useLocalUI();
    ...
    <>
      <header
        className={cn(
          ...
          scrolled
            ? "bg-pitch-900/95 backdrop-blur-md border-b border-neutral-700/50 shadow-lg"
            : "bg-pitch-900/80 backdrop-blur-sm"
        )}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    pathname === link.href
                      ...
                      : "text-neutral-300 hover:text-white hover:bg-pitch-800"
                  )}
                >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={openSearch}
                className="flex items-center gap-2 px-3 py-2 rounded-lg bg-pitch-800 hover:bg-pitch-700 border border-neutral-700/50 text-neutral-400 hover:text-white transition-all text-sm"
                aria-label="Search teams"
              >
                <Search className="w-4 h-4" />
                <span className="hidden sm:inline text-xs">Search</span>
                <kbd className="hidden sm:inline text-xs bg-pitch-700 px-1.5 py-0.5 rounded border border-neutral-600">
                  ⌘K
                </kbd>
              </button>
              ...
              <Link
                href="/teams"
                className="relative p-2 rounded-lg text-neutral-400 hover:text-amber-400 hover:bg-pitch-800 transition-all"
                aria-label="Favourite teams"
              >
              ...
              <button
                ...
                className="lg:hidden p-2 rounded-lg text-neutral-400 hover:text-white hover:bg-pitch-800 transition-all"
...
// Local UI state (avoids Zustand for simple toggle)
function useLocalUI() {
...
function SearchModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-pitch-950/80 backdrop-blur-sm flex items-start justify-center pt-20 px-4"
    ...
    >
      <div className="w-full max-w-xl bg-pitch-800 border border-neutral-700/50 rounded-2xl shadow-2xl overflow-hidden animate-slide-up">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-neutral-700/50">
          ...
          <kbd className="text-xs bg-pitch-700 text-neutral-400 px-2 py-1 rounded border border-neutral-600">
            ESC
          </kbd>
        </div>
        ...
        <div className="max-h-80 overflow-y-auto">
          ...
          {results.map((result) => (
            <Link
              ...
              href={result.href}
              onClick={onClose}
              className="flex items-center gap-3 px-4 py-3 hover:bg-pitch-700 transition-colors"
            >
              {result.image && (
              ...
              <div>
                ...
                {result.subtitle && (
                  <p className="text-neutral-400 text-xs">{result.subtitle}</p>
                )}
              </div>
              <span className="ml-auto text-xs text-neutral-500 capitalize bg-pitch-700 px-2 py-0.5 rounded">
                {result.type}
              </span>
            </Link>
          ))}
          {!query && (
            <div className="px-4 py-6 text-center">
              <p className="text-neutral-500 text-sm">
                Search for Premier League clubs, Champions League teams, and more
              </p>
            </div>
          )}
        </div>
