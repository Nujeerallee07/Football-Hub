// components/match/MatchLineup.tsx
"use client";

import Image from "next/image";
import { cn, getPositionColor, getPositionLabel, getFormationPositions } from "@/lib/utils";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <LineupList lineup={home} side="home" />
        <LineupList lineup={away} side="away" />
      </div>
    </div>
  );
}
...
function PitchFormation({
  ...
  const allPlayers = [
    ...homeLineup.startXI.slice(0, homePositions.length).map((p, i) => ({
      player: p.player,
      pos: homePositions[i],
      side: "home" as const,
    })),
    ...awayLineup.startXI.slice(0, awayPositions.length).map((p, i) => ({
      player: p.player,
      pos: awayPositions[i],
      side: "away" as const,
    })),
...
function LineupList({ lineup, side }: { lineup: MatchLineup; side: "home" | "away" }) {
  return (
    <div className="bg-pitch-800 rounded-xl border border-neutral-700/50 overflow-hidden">
      {/* Header */}
        ...
        <div className="flex items-center gap-2">
          {lineup.team.logo && (
            <div className="w-6 h-6 relative">
              <Image src={lineup.team.logo} alt={lineup.team.name} fill className="object-contain" />
            </div>
          )}
          <span className="text-white font-semibold text-sm">{lineup.team.name}</span>
        </div>
        <span className="text-neutral-400 text-xs bg-pitch-700 px-2 py-1 rounded">
          {lineup.formation}
        </span>
      </div>
      ...
      {lineup.substitutes.length > 0 && (
        <div className="p-3 border-t border-neutral-700/30 space-y-1">
          ...
          {lineup.substitutes.map(({ player }) => (
            <PlayerRow key={player.id} player={player} subdued />
          ))}
        </div>
      )}
    </div>
...
function PlayerRow({ player, subdued = false }: { player: Player; subdued?: boolean }) {
  return (
    <div className={cn("flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-pitch-700/50 transition-colors", subdued && "opacity-60")}>
      {player.number && (
        <span className="text-xs font-bold text-neutral-500 w-5 text-center flex-shrink-0">
          {player.number}
        </span>
      )}
