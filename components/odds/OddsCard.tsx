// components/odds/OddsCard.tsx
import { cn, formatOdds } from "@/lib/utils";
import type { ProcessedOdds } from "@/services/oddsApi";
...
export function OddsCard({ game, odds, className }: OddsCardProps) {
  ...
  const isToday =
    commenceDate.toDateString() === new Date().toDateString();

  return (
    <div
      className={cn(
        "bg-pitch-800 border border-neutral-700/50 rounded-xl overflow-hidden hover:border-neutral-600 transition-all duration-200 hover:shadow-card-hover",
        className
      )}
    >
      {/* Header */}
      ...
      <div className="p-4 grid grid-cols-3 gap-2">
        <OddsButton
          label="1"
          sublabel={game.home_team.split(" ").pop()!}
          odds={odds.homeWin}
          best={odds.bestHome}
          type="home"
        />
        ...
        <OddsButton
          label="2"
          sublabel={game.away_team.split(" ").pop()!}
          odds={odds.awayWin}
          best={odds.bestAway}
          type="away"
        />
  ...
  return (
    <button
      className={cn(
        ...
        isBest
          ? "border-grass-500/40 bg-grass-500/5"
          : "border-neutral-700/50 bg-pitch-700/30"
      )}
    >
      {isBest && (
        <div className="absolute top-1 right-1">
          <Star className="w-2.5 h-2.5 text-grass-400 fill-grass-400" />
        </div>
      )}
      ...
      <span
        className={cn(
          ...
          isBest ? "text-grass-400" : "text-white"
        )}
      >
        {formatOdds(odds)}
      </span>
      <span className="text-neutral-500 text-xs truncate w-full text-center">
        {sublabel}
      </span>
    </button>
  );
...
function BookmakerTable({
  ...
  bookmakers: ProcessedOdds["bookmakers"];
  homeTeam: string;
  awayTeam: string;
}) {
  const validBMs = bookmakers.filter(
            ...
            <tbody className="space-y-1">
              {validBMs.map((bm) => (
                <tr key={bm.name} className="border-t border-neutral-700/20">
                  ...
                  <td className={cn("py-1.5 text-center tabular-nums", bm.home ? "text-white" : "text-neutral-600")}>
                    {formatOdds(bm.home)}
                  </td>
                  <td className={cn("py-1.5 text-center tabular-nums", bm.draw ? "text-white" : "text-neutral-600")}>
                    {formatOdds(bm.draw)}
                  </td>
                  <td className={cn("py-1.5 text-center tabular-nums", bm.away ? "text-white" : "text-neutral-600")}>
                    {formatOdds(bm.away)}
                  </td>
                </tr>
              ))}
