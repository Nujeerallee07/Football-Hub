...
export function getPositionLabel(pos?: string): string {
  const labels: Record<string, string> = { G: "Goalkeeper", D: "Defender", M: "Midfielder", F: "Forward" };
...
export function getFormationPositions(
  ...
  allLines.forEach((count, lineIdx) => {
    const y = totalHeight - (lineIdx / (allLines.length - 1)) * totalHeight * 0.9 - totalHeight * 0.05;
...
export function teamLogoFallback(name: string): string {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=162d56&color=00e676&size=64&bold=true`;
}

export function playerPhotoFallback(name: string): string {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=1e2d3d&color=8ba5bd&size=64`;
}
