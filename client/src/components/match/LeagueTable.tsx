...
export function LeagueTable({ standings, compact = false, highlightTeamId, showForm = true, limit }: LeagueTableProps) {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-neutral-700/50 bg-pitch-800/80">
            ...
            {compact && <th className="text-center px-3 py-3 text-neutral-500 font-medium w-10">P</th>}
            <th className="text-center px-4 py-3 text-neutral-500 font-medium w-14">Pts</th>
            {showForm && !compact && <th className="text-center px-4 py-3 text-neutral-500 font-medium">Form</th>}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => {
            ...
            return (
              <tr
                ...
                className={cn(
                  ...
                  isHighlighted ? "bg-grass-500/10" : i % 2 === 0 ? "bg-pitch-800/40 hover:bg-pitch-800/70" : "bg-pitch-800/20 hover:bg-pitch-800/50"
                )}
              >
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    {descColor && <div className={cn("w-1 h-5 rounded-full flex-shrink-0", descColor)} />}
                    <span className={cn("font-medium text-sm", row.rank <= 4 ? "text-grass-400" : "text-neutral-400")}>
                      {row.rank}
                    </span>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <Link to={`/teams/${row.team.id}`} className="flex items-center gap-2.5 hover:text-white group/team">
                    <img src={row.team.logo} alt={row.team.name} className="w-6 h-6 object-contain flex-shrink-0" />
                    <span className={cn("font-medium group-hover/team:text-white transition-colors", isHighlighted ? "text-white" : "text-neutral-200")}>
                      {row.team.name}
                    </span>
                  </Link>
                </td>
                {!compact && (
                  <>
                    ...
                    <td className="px-3 py-3 text-center tabular-nums">
                      <span className={cn("font-medium", row.goalsDiff > 0 ? "text-grass-400" : row.goalsDiff < 0 ? "text-red-400" : "text-neutral-400")}>
                        {row.goalsDiff > 0 ? `+${row.goalsDiff}` : row.goalsDiff}
                      </span>
                    </td>
                  </>
                )}
                {compact && <td className="px-3 py-3 text-center text-neutral-400 tabular-nums">{row.all.played}</td>}
                <td className="px-4 py-3 text-center">
                  <span className={cn("font-bold tabular-nums", isHighlighted ? "text-grass-400" : "text-white")}>{row.points}</span>
                </td>
                {showForm && !compact && row.form && (
                  <td className="px-4 py-3"><FormBadges form={row.form} /></td>
                )}
              </tr>
            );
          })}
...
function FormBadges({ form }: { form: string }) {
  ...
  return (
    <div className="flex gap-1 justify-center">
      {results.map((r, i) => (
        <span key={i} className={cn("w-5 h-5 rounded-sm text-xs font-bold flex items-center justify-center text-pitch-950", getFormColor(r))}>
          {r}
        </span>
      ))}
    </div>
