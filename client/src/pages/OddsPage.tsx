...
import { SPORT_KEYS, processOdds, type SportKey } from "@/services/oddsApi";
...
export function OddsPage() {
  const [selectedSport, setSelectedSport] = useState<SportKey>(SPORT_KEYS.PREMIER_LEAGUE);
  const { data: games, isLoading, isError, refetch } = useOdds(selectedSport);

  return (
    <div className="page-container space-y-8">
      <div>
        <h1 className="font-display text-3xl font-bold text-white mb-2">Betting Odds</h1>
        <p className="text-neutral-400">Best UK bookmaker odds — updated every 2 minutes</p>
      </div>

      <div className="flex items-start gap-3 p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
        <AlertCircle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" />
        <p className="text-amber-300/80 text-sm">
          <strong>18+ Gamble Responsibly.</strong> Odds are for informational purposes only. Please visit{" "}
          <a href="https://www.begambleaware.org" target="_blank" rel="noopener noreferrer" className="underline hover:text-amber-300">
            BeGambleAware.org
          </a>{" "}
      ...
      {isError && <ErrorState message="Failed to load odds. Please check your API key." onRetry={() => refetch()} />}
