...
const LEAGUE_OPTIONS = [
  { id: PL_ID, label: "Premier League" },
  { id: UCL_ID, label: "Champions League" },
];

export function TeamsPage() {
  const [selectedLeague, setSelectedLeague] = useState(PL_ID);
  ...
  const standingMap = useMemo(() => {
    ...
    return new Map<number, { rank: number; points: number; form: string }>(
      standings.map((s: any) => [s.team.id, { rank: s.rank, points: s.points, form: s.form }])
    );
  }, [standingsData]);
  ...
  const filteredTeams = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      teams = teams.filter((t: any) =>
        t.team.name.toLowerCase().includes(q) || t.team.country?.toLowerCase().includes(q)
      );
    }
  ...
  return (
    <div className="page-container space-y-8">
      <div>
        <h1 className="font-display text-3xl font-bold text-white mb-2">Teams</h1>
        <p className="text-neutral-400">Premier League & Champions League clubs</p>
      </div>
      ...
      <div className="space-y-6">
            ...
            {LEAGUE_OPTIONS.map((opt) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  selectedLeague === opt.id ? "bg-pitch-600 text-white" : "text-neutral-400 hover:text-white"
                }`}
              >
                {opt.label}
              </button>
        ...
        {!isLoading && filteredTeams.length > 0 && (
          <p className="text-neutral-500 text-sm">{filteredTeams.length} club{filteredTeams.length !== 1 ? "s" : ""}</p>
        )}

        {isLoading && <SkeletonGrid count={20} />}
        {isError && <ErrorState message="Failed to load teams." onRetry={() => refetch()} />}
